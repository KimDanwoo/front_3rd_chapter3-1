...
describe('getUpcomingEvents', () => {
  it('알림 시간이 정확히 도래한 이벤트를 반환한다', () => {
    ...
    const upcomingEvents = getUpcomingEvents(FILTERED_EVENTS, now, []);
    expect(upcomingEvents).toHaveLength(1);
  });

  it('이미 알림이 간 이벤트는 제외한다', () => {
    const now = new Date('2024-08-15T15:00');
    const upcomingEvents = getUpcomingEvents(FILTERED_EVENTS, now, ['1']);
...
describe('createNotificationMessage', () => {
  it('올바른 알림 메시지를 생성해야 한다', () => {
    const message = createNotificationMessage(FILTERED_EVENTS[0]);
